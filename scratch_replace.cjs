...
function processDir(dir) {
  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      // additional fallback fixes for gradients
      content = content.replace(/bg-gradient-to-r/g, 'bg-cyan-500/10');
      content = content.replace(/bg-gradient-to-b/g, 'bg-cyan-500/10');
      content = content.replace(/bg-gradient-to-br/g, 'bg-cyan-500/10');
      content = content.replace(/bg-gradient-to-tr/g, 'bg-cyan-500/10');
      content = content.replace(/bg-gradient-to-bl/g, 'bg-cyan-500/10');
      content = content.replace(/bg-gradient-to-tl/g, 'bg-cyan-500/10');
